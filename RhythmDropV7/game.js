...
//
// So the collapse is continuous and scroll-linked instead: for scroll
// y the header gives up exactly min(y, C) pixels, where C is measured.
//
// The trick that makes it not shift anything is the pane's padding.
// Collapsing by c moves the pane's top edge up by c, which on its own
// would move the content twice per scrolled pixel. Growing the pane's
// own padding-top by the same c puts those pixels back inside the
// scroller: a row at offset d renders at (top0 - c) + (pad0 + c) - y
// + d, which is top0 + pad0 + d - y — independent of c. Every pixel of
// collapse is paid for by a pixel of scroll, so the content moves
// exactly 1:1 with the finger. This is how iOS large-title headers
// avoid the same jump.
//
// Doing it in padding rather than by pinning scrollTop matters: a
// pinned scroller sits at 0, and scrolling up from 0 fires no scroll
// event at all, so the header could collapse and never come back.
// Here scrollTop is never touched, so the browser drives both
// directions natively.
window.RD_Header = (function wireCompactHeader() {
  ...
  let panePad = 0;    // the panes' resting padding-top
  ...
  function measure() {
    ...
    panes().forEach(p => { p.style.paddingTop = ''; });
    ...
    panePad = anyPane ? (parseFloat(getComputedStyle(anyPane).paddingTop) || 0) : 0;
  ...
  function apply(c) {
    ...
    // pinned in px below whatever it does inside.
    ...
    // The pixels the header gave up, handed to the scroller so the
    // content stays exactly where the scroll position says it is.
    panes().forEach(p => { p.style.paddingTop = (panePad + c) + 'px'; });
  }

  // Reads the collapse straight off the active pane's scroll position,
  // so there is no state of our own to get out of step with it.
  function sync() {
    const pane = document.querySelector('.tab-pane.active') || document.querySelector('.tab-pane');
    const y = pane ? pane.scrollTop : 0;
    apply(Math.max(0, Math.min(y, C)));
  }
  ...
  panes().forEach(pane => {
    pane.addEventListener('scroll', () => {
      ...
      apply(Math.max(0, Math.min(pane.scrollTop, C)));
    }, { passive: true });
  });
  ...
  return { measure, reset, apply, sync,
    get total() { return C; },
...
document.querySelectorAll('.hnav').forEach(t => t.addEventListener('click', () => {
  ...
  // Each pane keeps its own scroll, but the header is shared — so on a
  // tab switch the header takes its state from the pane now on screen
  // rather than inheriting a collapse that pane never paid for.
  if (window.RD_Header) window.RD_Header.sync();
  uiSound('detent');
